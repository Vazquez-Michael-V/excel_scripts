...
def check_columns(df_main_file, substring_columns):
    """Checks that the given list of columns names are in fact column names in the DataFrame.\n
    Intended to avoid a KeyError exception."""
    
    try:
        for s in substring_columns:        
            df_temp_cols_check = df_main_file[s]     
            print(df_temp_cols_check)
    
    except KeyError as e: 
        print(traceback.format_exception_only(e.__class__, e))
        print(f"Column name {e} not found.")   
        return False
    else:
        return True
...
def find_contents(specified_column, substring_columns, excel_filename, results_filename=None):
    ...
    if check_columns(df_main_file, substring_columns) == True:
